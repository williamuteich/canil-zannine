...
export default async function NossosFilhotesPage() {
  let puppies: Puppy[] = [];

  try {
    const result = await getData<{ data: Puppy[] }>("/api/filhote");
  ...
  const activePuppies = puppies.filter(p => p.status === 'ativo');

  const displayPuppies = activePuppies;
